...
| Version | Key Changes |
|---------|-------------|
| v6.14 | **OBD oil temp via Mode 22 + watchdog/power-loss hardening (2026-07-30; host-verified logic only, NOT flash-tested; not committed).** (1) **Oil temp over OBD**: generic PID 0x5C is unsupported on the VQ37 (logs showed `OilTf:-1`), but the 370Z exposes it via **Nissan Mode 0x22, DID 0x111F, physical addr 7E0, °C = A−50** (web-confirmed on 370Z forums). Added `sendOBD_Mode22()`/`decodeOBD_Mode22Reply()` (routes service-byte 0x62 in the RX drain), polled ~1 Hz to 7E0. `mergeOilTempSource()` (loop step 1.5, between updateVehicleData and updateUI) drives the oil-temp **dial** from the OBD value whenever the dead Modbus PRTXI sensor is offline — order-safe, no UI surgery; the physical sensor wins if it's ever alive. **Validate the −50 offset vs the factory gauge on first drive.** (2) **Task watchdog**: `esp_task_wdt` armed on the main loop (`WDT_TIMEOUT_MS=12000`, `#if ESP_IDF_VERSION_MAJOR>=5` reconfigure else init), `esp_task_wdt_add(NULL)` in setup, `esp_task_wdt_reset()` at loop top — a hang auto-resets instead of freezing. No hardware. (3) **Power-loss flush**: `ENABLE_POWER_FAIL_DETECT` (default **0/off**) + `checkPowerFail()` reads a 12V-sense divider on `POWER_SENSE_PIN` (default GPIO6); on power good→failing it `sdSafeFlush()`es so a mid-write key-off can't corrupt the card. Dormant until wired + enabled. `[WDT]`/`[PWR]` tags. Verified: braces/parens/#if balanced, CSV still 66 cols. |
